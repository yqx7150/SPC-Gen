...
if __name__ == "__main__":
    root = "/mnt/D/yxb/stable-diffusion-master/logs/2025-04-03T14-17-03_CT_PET_512_L_VQ/images/train/split"
    n = 0  # 要补齐的列数
    # 处理之后的数据
    out_images = list(filter(lambda x:x.endswith("png"), os.listdir(root)))
    # 读取投影域图片
    for i, projection_image in enumerate(out_images):
        projection_image = Image.open(os.path.join(root, projection_image))
        projection_image = np.array(projection_image).astype(np.float32)
        projection_image = get_gray(projection_image, 3)
        resolution = 512
