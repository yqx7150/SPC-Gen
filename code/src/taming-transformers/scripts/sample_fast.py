...
def sample_unconditional(model, batch_size, steps=256, temperature=None, top_k=None, top_p=None, callback=None,
    ...
    log = dict()
    qzshape = [batch_size, dim_z, h, w]
    assert model.be_unconditional, 'Expecting an unconditional models.'
...
def load_model(config, ckpt, gpu, eval_mode):
    ...
    if ckpt:
        ...
        print(f"loaded models from global step {global_step}.")
    else:
        pl_sd = {"state_dict": None}
        global_step = None
    model = load_model_from_config(config.model, pl_sd["state_dict"], gpu=gpu, eval_mode=eval_mode)["models"]
...
if __name__ == "__main__":
    ...
    if os.path.isfile(opt.resume):
        ...
        try:
            ...
            idx = -2 # take a guess: path/to/logdir/checkpoints/models.ckpt
