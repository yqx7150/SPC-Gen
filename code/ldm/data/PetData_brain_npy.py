...
class PET_SIN_512_train_brain(PETBase):
    def __init__(self, **kwargs):
        ...
        super().__init__(txt_file="/mnt/D/ywy/ALL_PET/Trunk_pet_dataest/dataest_npy/train.txt", 
                         data_root="/mnt/D/ywy/ALL_PET/Trunk_pet_dataest/dataest_npy/train",** kwargs)
...
class PET_SIN_512_val_brain(PETBase):
    def __init__(self, **kwargs):
        if 'config' in kwargs:
            kwargs.pop('config')
        # 注意：确保txt文件中存储的是NPY文件的路径（例如xxx.npy）
        super().__init__(txt_file="/mnt/D/ywy/ALL_PET/Trunk_pet_dataest/dataest_npy/val.txt", 
                         data_root="/mnt/D/ywy/ALL_PET/Trunk_pet_dataest/dataest_npy/val",** kwargs)