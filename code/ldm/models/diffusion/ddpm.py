...
class Layout2ImgDiffusion(LatentDiffusion):
    ...
    def log_images(self, batch, N=8, *args, **kwargs):
        ...
        key = 'train' if self.training else 'val'  # 训练时读取train数据集，验证时读取validation数据集
...
#
#     """
#         can either run as pure inpainting models (only concat mode) or with mixed conditionings,
...
class LatentInpaintDiffusion(LatentDiffusion):
    """
    can just run as pure inpainting models (only concat mode).
    To disable finetuning mode, set finetune_keys to None
     """
    def __init__(self,
                 # DEFAULT FINETUNE KEYS --> use to add novel channels to the concatenation
                 finetune_training_keys=("models.diffusion_model.input_blocks.0.0.weight",
                                         "model_ema.diffusion_modelinput_blocks00weight"
                                         ),
                 finetune_keys_to_retain=("models.diffusion_model.input_blocks.0.0.weight",
                                          "model_ema.diffusion_modelinput_blocks00weight"
                                          ),
                 concat_keys=("masked_image", "mask"),  # AS IN INFERENCE
                 masked_image_key="masked_image",
                 #  keep_finetune_dims=4,  # if models was trained without concat mode before and we would like to keep these channels
                 keep_finetune_dims=7,
                 # if models was trained with concat mode before and we would like to keep these channels and add more channels to condition
